...
class TasksMixIn:
    ...
    def set_task_status(self, task: Task, status) -> Task:
        ...
        if status in (TASK_RUNNING, TASK_DISTRIBUTED):
            task.started_on = _utcnow_naive()
            if not task.analysis_started_on:
                task.analysis_started_on = task.started_on
        elif status in (TASK_COMPLETED, TASK_DISTRIBUTED_COMPLETED):
            task.completed_on = _utcnow_naive()
            if not task.analysis_finished_on:
                task.analysis_finished_on = task.completed_on
    ...
    def mark_task_stage(self, task_id: int, stage: str, event: str = "started") -> Optional[Task]:
        ...
        now = _utcnow_naive()
        field_map = {
            ("analysis", "started"): "analysis_started_on",
            ("analysis", "finished"): "analysis_finished_on",
            ("processing", "started"): "processing_started_on",
            ("processing", "finished"): "processing_finished_on",
            ("signatures", "started"): "signatures_started_on",
            ("signatures", "finished"): "signatures_finished_on",
            ("reporting", "started"): "reporting_started_on",
            ("reporting", "finished"): "reporting_finished_on",
        }

        field_name = field_map.get((stage, event))
        if not field_name:
            return task

        setattr(task, field_name, now)
        self.session.add(task)
        return task
