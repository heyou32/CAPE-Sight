...
def tasks_status(request, task_id):
    ...
    if request.method == "GET":
        ...
        if request.query_params.get("detailed") == "1":
            resp = {"error": False, "data": _estimate_task_timing(task)}
        else:
...
def _timedelta_seconds(started_at, finished_at=None):
    if not started_at:
        return None
...
def _compute_entropy_for_task(task):
    if not getattr(task, "sample", None) or not task.sample.sha256:
        return None

    sample_paths = db.sample_path_by_hash(task.sample.sha256, task.id)
    if not sample_paths:
        return None
...
def _classify_task_profile(task):
    if task.category != "file" or not getattr(task, "sample", None):
        return task.category or "unknown"

    file_type = (task.sample.file_type or "").lower()
    if any(token in file_type for token in ("dll", "shared object")):
        return "dll"
    if any(token in file_type for token in ("pe32", "executable", "ms-dos")):
        return "exe"
    if any(token in file_type for token in ("pdf", "word", "excel", "powerpoint", "rtf", "office")):
        return "document"
    if any(token in file_type for token in ("script", "javascript", "python", "powershell", "vbs", "shell")):
        return "script"
    return "other"


def _trimmed_average(samples, fallback):
    if not samples:
        return fallback
    ...
    if len(ordered) >= 6:
        ...
        ordered = ordered[trim_size:-trim_size] or ordered

    return max(1, int(mean(ordered)))


def _calculate_running_estimate(task, stage_averages, entropy=None):
    running_estimate = stage_averages["running"]
    if task.timeout:
        running_estimate = int((running_estimate * 0.7) + (min(task.timeout, max(running_estimate * 2, 60)) * 0.3))

    file_size = getattr(getattr(task, "sample", None), "file_size", None)
    if file_size:
        running_estimate += min(45, int(file_size / (1024 * 1024 * 5)))
    if entropy is not None:
        running_estimate += max(0, int((entropy - 6.0) * 6))

    return max(30, running_estimate)


def _collect_stage_averages(task, limit=80):
    stage_samples = {stage: [] for stage in ETA_DEFAULTS}
    task_profile = _classify_task_profile(task)
    recent_tasks = db.list_tasks(
        limit=limit,
        status=f"{TASK_REPORTED}|{TASK_RECOVERED}",
        include_hashes=True,
        order_by=Task.added_on.desc(),
    )

    for recent_task in recent_tasks:
        if recent_task.category != task.category:
            continue
        if _classify_task_profile(recent_task) != task_profile:
            continue

        pending_seconds = _timedelta_seconds(recent_task.added_on, recent_task.started_on)
        running_seconds = _timedelta_seconds(
            recent_task.analysis_started_on or recent_task.started_on,
            recent_task.analysis_finished_on or recent_task.completed_on,
        )
        processing_seconds = _timedelta_seconds(recent_task.processing_started_on, recent_task.processing_finished_on)
        reporting_seconds = _timedelta_seconds(recent_task.reporting_started_on, recent_task.reporting_finished_on)

        if pending_seconds is not None:
            stage_samples["pending"].append(pending_seconds)
        if running_seconds is not None:
            stage_samples["running"].append(running_seconds)

        if processing_seconds is not None:
            stage_samples["processing"].append(processing_seconds)
        if reporting_seconds is not None:
            stage_samples["reporting"].append(reporting_seconds)

        if processing_seconds is None and recent_task.completed_on and recent_task.reporting_started_on:
            fallback_processing = _timedelta_seconds(recent_task.completed_on, recent_task.reporting_started_on)
            if fallback_processing is not None:
                stage_samples["processing"].append(fallback_processing)
        if reporting_seconds is None and recent_task.reporting_started_on and recent_task.reporting_finished_on:
            fallback_reporting = _timedelta_seconds(recent_task.reporting_started_on, recent_task.reporting_finished_on)
            if fallback_reporting is not None:
                stage_samples["reporting"].append(fallback_reporting)

    averages = ETA_DEFAULTS.copy()
    for stage, samples in stage_samples.items():
        averages[stage] = _trimmed_average(samples, averages[stage])

    return averages


def _get_stage_for_task(task):
    if task.status == TASK_PENDING:
        return "pending"
    if task.status in (TASK_RUNNING, TASK_DISTRIBUTED):
        return "running"
    if task.status == TASK_COMPLETED:
        if getattr(task, "reporting_started_on", None) and not getattr(task, "reporting_finished_on", None):
            return "reporting"
        if getattr(task, "processing_started_on", None) and not getattr(task, "processing_finished_on", None):
            return "processing"
        return "processing"
    if task.status == TASK_FAILED_PROCESSING:
        return "processing"
    if task.status == TASK_FAILED_REPORTING:
        return "reporting"
    if task.status in (TASK_REPORTED, TASK_RECOVERED):
        return "reported"
    return task.status


def _get_stage_started_at(task, stage):
    if stage == "pending":
        return task.added_on
    if stage == "running":
        return task.analysis_started_on or task.started_on or task.added_on
    if stage == "processing":
        return task.processing_started_on or task.completed_on or task.started_on
    if stage == "reporting":
        return task.reporting_started_on or task.processing_finished_on or task.completed_on
    return None


def _estimate_queue_wait(task, stage_averages, active_slots):
    pending_tasks = db.list_tasks(status=TASK_PENDING, include_hashes=True, order_by=Task.added_on.asc())
    ahead_tasks = [pending_task for pending_task in pending_tasks if pending_task.added_on < task.added_on]
    if not ahead_tasks:
        return 0, 0

    total_ahead_seconds = 0
    for ahead_task in ahead_tasks:
        total_ahead_seconds += _calculate_running_estimate(ahead_task, stage_averages)

    return len(ahead_tasks), int(total_ahead_seconds / max(active_slots, 1))


def _estimate_task_timing(task):
    stage_averages = _collect_stage_averages(task)
    current_stage = _get_stage_for_task(task)
    queue_position = 0
    entropy = None
    file_size = None
    active_slots = max(len(db.list_machines()), 1)

    if getattr(task, "sample", None):
        file_size = task.sample.file_size
        entropy = _compute_entropy_for_task(task)

    stage_averages["running"] = _calculate_running_estimate(task, stage_averages, entropy)

    if current_stage == "pending":
        queue_position, estimated_queue_wait = _estimate_queue_wait(task, stage_averages, active_slots)
        stage_averages["pending"] = max(stage_averages["pending"], estimated_queue_wait)

    stage_order = ["pending", "running", "processing", "reporting"]
    if current_stage not in stage_order:
        return {
            "status": task.status,
            "stage": current_stage,
            "stage_label": current_stage.replace("_", " ").title(),
            "progress": 100 if current_stage == "reported" else 0,
            "estimated_total": 0,
    ...
    total_estimate = sum(stage_averages[stage] for stage in stage_order)
    completed_estimate = 0
    elapsed_current = 0

    for stage in stage_order:
        if stage == current_stage:
            started_at = _get_stage_started_at(task, stage)
            elapsed_current = min(stage_averages[stage], _timedelta_seconds(started_at) or 0)
            break
        completed_estimate += stage_averages[stage]

    remaining = max(0, total_estimate - completed_estimate - elapsed_current)
    progress = int(min(99, ((completed_estimate + elapsed_current) / max(total_estimate, 1)) * 100))
    ...
    return {
        "status": task.status,
        "stage": current_stage,
        "stage_label": stage_labels.get(current_stage, current_stage.replace("_", " ").title()),
        "progress": progress,
        "estimated_total": total_estimate,
        "estimated_remaining": remaining,
        "elapsed": completed_estimate + elapsed_current,
        "queue_position": queue_position,
        "file_size": file_size,
        "entropy": entropy,
    }
...
def _libvirt_connection():
    try:
        ...
        machinery_cfg = Config(machinery_name).get(machinery_name)
        dsn = getattr(machinery_cfg, "dsn", None)
    except Exception:
        dsn = None

    try:
        return libvirt.open(dsn) if dsn else libvirt.open(None)
    except Exception:
        return None


def _libvirt_cpu_percent(label, cpu_time_ns):
    previous = _LIBVIRT_CPU_HISTORY.get(label)
    now = time.time()
    _LIBVIRT_CPU_HISTORY[label] = (cpu_time_ns, now)
    ...
    cpu_count = psutil.cpu_count(logical=True) if HAVE_PSUTIL else 1
    percent = ((cpu_time_ns - previous_cpu) / 1e9) / elapsed * 100.0 / max(cpu_count, 1)
    return round(percent, 1)


def _get_libvirt_machine_stats():
    ...
    for machine in db.list_machines():
        try:
            vm = conn.lookupByName(machine.label)
            info = vm.info()
            state = info[0]
            status = {
                1: "running",
            ...
            max_memory_mb = int(info[1] / 1024) if info[1] else None
            memory_mb = int(info[2] / 1024) if info[2] else None
            memory_percent = None
            if max_memory_mb and memory_mb is not None:
                memory_percent = round(memory_mb / max_memory_mb * 100, 1)

            cpu_percent = None
            if state == 1:
                cpu_percent = _libvirt_cpu_percent(machine.label, int(info[4]))
            ...
            stats.append(
                {
                    ...
                    "name": machine.name,
                    "status": status,
                    "cpu_percent": f"{cpu_percent}%" if cpu_percent is not None else None,
                    "memory": {
                        "assigned": memory_mb,
                        "total": max_memory_mb,
                        "percent": f"{memory_percent}%" if memory_percent is not None else None,
                    },
                }
            )
        except Exception:
...
def cuckoo_status(request):
    ...
    if not apiconf.cuckoostatus.get("enabled"):
        ...
        if HAVE_LIBVIRT:
            resp["data"]["machines"] = _get_libvirt_machine_stats()
    return Response(resp)
