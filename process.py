...
def process(
    ...
    try:
        with db.session.begin():
            db.mark_task_stage(task_id, "processing", "started")
        if memory_debugging:
            gc.collect()
        ...
        with db.session.begin():
            ...
            db.mark_task_stage(task_id, "processing", "finished")
            db.mark_task_stage(task_id, "signatures", "started")
        if memory_debugging:
            gc.collect()
        ...
        with db.session.begin():
            db.mark_task_stage(task_id, "signatures", "finished")
        if memory_debugging:
            gc.collect()
        ...
        if report:
            ...
            with db.session.begin():
                db.mark_task_stage(task_id, "reporting", "started")
            ...
            with db.session.begin():
                db.mark_task_stage(task_id, "reporting", "finished")
        ...
        log.exception("Task #%d processing pipeline failed: %s", task_id, error)
        with db.session.begin():
            db_task = db.view_task(task_id)
            if db_task:
                if db_task.processing_started_on and not db_task.processing_finished_on:
                    db.mark_task_stage(task_id, "processing", "finished")
                if db_task.signatures_started_on and not db_task.signatures_finished_on:
                    db.mark_task_stage(task_id, "signatures", "finished")
                if db_task.reporting_started_on and not db_task.reporting_finished_on:
                    db.mark_task_stage(task_id, "reporting", "finished")
